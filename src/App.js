  ...
  addedCoffee(item, index) {
    return (
      <div className="coffee__item" key={index}>
        <button onClick={e => this.deleteCoffeeItem(e, index)}>
          <i className="fa fa-times fa-2x icon"></i>
        </button>
      ...
      <div className="container">
        ...
        <div className="form">
          <div className="form__title">
            ...
            <button className="button button--add" onClick={this.showForm}>
              {!this.state.showForm ? "+" : "-"}
            </button>
          </div>
          <form
            className={
              this.state.showForm ? "form__container:active" : "form__container"
            }
        ...
        <div className="coffee">
          {this.props.allCoffeeItems &&
            this.props.allCoffeeItems.map((item, index) =>
              this.addedCoffee(item, index)
            )}
        </div>
      </div>
    );
...
const mapDispatchToProps = dispatch => {
  return {
    createCoffeeItem: coffeeItem =>
      dispatch(coffeeItemAction.createNewCoffeeItem(coffeeItem)),
    deleteCoffeeItem: index =>
      dispatch(coffeeItemAction.deleteCoffeeItem(index)),
    loadDataFromLocalStorage: () =>
      dispatch(coffeeItemAction.loadDataFromLocalStorage())
  };
};
